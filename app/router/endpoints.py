...
async def map_plan(customer: schemas.CustomerSubscribe, db: Session):
    'res=db.query(models.Customer_plan).where(models.Customer_plan.customer_id == customer.customer_id)'
    res=db.query(models.Customer_plan).filter(models.Customer_plan.customer_id == customer.customer_id).first()
    if res is None:
...
async def get_customer_plan(id: schemas.CustomerRegister, db: Session):
    res=db.query(models.Customer_plan).filter(models.Customer_plan.customer_id == id.customer_id)
    if res is None:
